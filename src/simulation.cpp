...
Simulation::Simulation() { simulation_flip_flop = 1; }
  ...
  if (simulation_flip_flop) {
    if (grid[x][y].id == 1 && y + 1 < GRID_SIZE) {
      if (grid[x][y + 1].id == 0 || grid[x][y + 1].id == 2) {
        ...
        grid[x][y + 1] = grid[x][y];
        grid[x][y] = temp;
      } else if (x > 0 && grid[x - 1][y + 1].id == 0 ||
                 x > 0 && grid[x - 1][y + 1].id == 2) {
        temp = grid[x - 1][y + 1];
        grid[x - 1][y + 1] = grid[x][y];
        grid[x][y] = temp;
      } else if (x + 1 < GRID_SIZE && grid[x + 1][y + 1].id == 0 ||
                 x + 1 < GRID_SIZE && grid[x + 1][y + 1].id == 2) {
        temp = grid[x + 1][y + 1];
        grid[x + 1][y + 1] = grid[x][y];
        grid[x][y] = temp;
      }
      ...
      if (y + 1 < GRID_SIZE && grid[x][y + 1].id == 0) {
        ...
        grid[x][y + 1] = grid[x][y];
        grid[x][y] = temp;
      } else if (x + 1 < GRID_SIZE && y + 1 < GRID_SIZE &&
                 grid[x + 1][y + 1].id == 0) {
        temp = grid[x + 1][y + 1];
        grid[x + 1][y + 1] = grid[x][y];
        grid[x][y] = temp;
